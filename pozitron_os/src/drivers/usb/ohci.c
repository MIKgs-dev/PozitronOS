...
int ohc_reset(uchar controller)
{
        ...
        if (readl(&ohci_regs->control) & OHCI_CTRL_IR) {
            writel(OHCI_OCR, &ohci_regs->cmdstatus);
            int timeout = 50; /* 500ms */
            while ((readl(&ohci_regs->control) & OHCI_CTRL_IR) && timeout) {
                mdelay(10);
                timeout--;
            }
            if (!timeout) {
                /* SMM не отвечает — форсируем */
                writel(OHCI_HCR, &ohci_regs->cmdstatus); /* Reset HC */
                mdelay(10);
...
{       
	...
	int size;
        ...
        dword = pci_read32(dev->bus, dev->device, dev->func, 0x10);
...
//	init urb and urb_priv
	...
	size = 4;
        ...
        // set master
        word = pci_read16(dev->bus, dev->device, dev->func, 0x04);
        word |= 0x04;
        pci_write16(dev->bus, dev->device, dev->func, 0x04, word);
