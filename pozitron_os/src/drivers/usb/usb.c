...
void hci_init(void)
{
    ...
    // Ищем UHCI (prog_if = 0x00)
    count = pci_find_all_class(0x0c, 0x03, 0x00, devices, 8);
    for(i=0; i<count && num_controllers < MAX_CONTROLLERS; i++) {
        hc_type[num_controllers] = 0x00;
        uhc_init(&devices[i]);
        num_controllers++;
    }

    // Ищем OHCI (prog_if = 0x10)
    count = pci_find_all_class(0x0c, 0x03, 0x10, devices, 8);
    for(i=0; i<count && num_controllers < MAX_CONTROLLERS; i++) {
        hc_type[num_controllers] = 0x10;
        ohc_init(&devices[i]);
        num_controllers++;
    }
