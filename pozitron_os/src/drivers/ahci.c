...
#define AHCI_REQUEST_TIMEOUT  3200
#define AHCI_RESET_TIMEOUT     500
#define AHCI_LINK_TIMEOUT       10
...
static void sata_prep_simple(struct sata_cmd_fis* fis, uint8_t command) {
...
static void sata_prep_atapi(struct sata_cmd_fis* fis, uint16_t blocksize) {
    memset(fis, 0, sizeof(*fis));
    fis->command = ATA_CMD_PACKET;
    fis->feature = 1;
    fis->lba_mid = blocksize & 0xFF;
    fis->lba_high = (blocksize >> 8) & 0xFF;
}

static int ahci_command(struct ahci_port* port, int iswrite, int isatapi,
                        void* buffer, uint32_t bsize) {
    uint32_t val, status, flags, intbits, error;
    ...
    uint32_t end = timer_calc_ms(AHCI_REQUEST_TIMEOUT);
    do {
        for (;;) {
            intbits = ahci_port_readl(pnr, PORT_IRQ_STAT);
            if (intbits) {
    ...
    if (!success) {
        serial_puts("[AHCI] Command error\n");
        ...
        val = ahci_port_readl(pnr, PORT_TFDATA);
        if (val & (ATA_CB_STAT_BSY | ATA_CB_STAT_DRQ)) {
            val = ahci_port_readl(pnr, PORT_SCR_CTL);
            ahci_port_writel(pnr, PORT_SCR_CTL, val | 1);
            mdelay(1);
            ahci_port_writel(pnr, PORT_SCR_CTL, val);
        }
        val = ahci_port_readl(pnr, PORT_CMD);
        ahci_port_writel(pnr, PORT_CMD, val | PORT_CMD_START);
    }
...
static int ahci_port_setup(uint32_t pnr) {
    ...
    end = timer_calc_ms(AHCI_REQUEST_TIMEOUT);
    for (;;) {
    ...
    int rc = ahci_command(port, 0, 1, buffer, sizeof(buffer));
...
int ahci_read_sectors(int port, uint64_t lba, uint32_t count, void* buffer) {
    ...
    int rc = ahci_command(p, 0, 0, buffer, count * p->sector_size);
    return rc == 0 ? count * p->sector_size : -1;
}
...
int ahci_write_sectors(int port, uint64_t lba, uint32_t count, void* buffer) {
    ...
    int rc = ahci_command(p, 1, 0, buffer, count * p->sector_size);
    return rc == 0 ? count * p->sector_size : -1;
}
