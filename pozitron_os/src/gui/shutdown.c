...
void shutdown_dialog_callback(Widget* button, void* userdata) {
    ...
    wg_create_label(shutdown_dialog, "Are you sure you want to", 0.2f, 0.25f);
    wg_create_label(shutdown_dialog, "shutdown the computer?", 0.22f, 0.4f);
    
    Widget* btn_yes = wg_create_button(shutdown_dialog, "Yes", 
                                       0.25f, 0.65f, 0.2f, 0.2f,
                                       shutdown_confirm_callback, NULL);
    
    Widget* btn_no = wg_create_button(shutdown_dialog, "No",
                                      0.55f, 0.65f, 0.2f, 0.2f,
