...
struct Window {
    uint32_t id;
    char* title;
    int32_t x, y;                          // ← ИЗМЕНЕНО: были uint32_t, стали int32_t
    ...
    uint8_t dragging;
    uint8_t resizing;
    int32_t drag_offset_x, drag_offset_y;   // ← ИЗМЕНЕНО
    uint32_t resize_corner;
    int32_t resize_start_x, resize_start_y; // ← ИЗМЕНЕНО
    uint32_t resize_start_width, resize_start_height;
    uint8_t minimized;
    uint8_t maximized;
    int32_t orig_x, orig_y;                 // ← ИЗМЕНЕНО
    uint32_t orig_width, orig_height;
    int32_t normal_x, normal_y;              // ← ИЗМЕНЕНО
