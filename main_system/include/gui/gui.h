...
struct Window {
    ...
    uint32_t* canvas_back;
    uint32_t* canvas_front;
    uint32_t canvas_width;
    uint32_t canvas_height;
    uint32_t canvas_capacity;
    uint8_t canvas_enabled;
    uint8_t canvas_swap_pending;
    event_t win_events[WIN_EVENT_QUEUE_SIZE];
    uint8_t win_event_head;
    uint8_t win_event_tail;
    uint8_t win_event_count;
    input_event_t input_queue[128];
    uint8_t input_head;
    uint8_t input_tail;
    uint8_t input_count;
...
Widget* wg_create_button_aligned(Window* parent, const char* text,
                                 float rel_x, float rel_y,
                                 float rel_width, float rel_height,
                                 uint8_t align, void (*callback)(Widget*, void*), void* userdata);
