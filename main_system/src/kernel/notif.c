...
#define COLOR_MODAL_BG  0xEAEAEA
...
typedef struct {
    ...
    uint8_t mouse_down;
    int clicked_index;
    uint8_t clicked_body;

    uint8_t modal_active;
    notif_t modal_source;
    int modal_src_index;
    uint32_t modal_freeze_tick;
    
    uint32_t modal_x, modal_y, modal_w, modal_h;
    uint32_t modal_btn_x, modal_btn_y, modal_btn_w, modal_btn_h;
} notif_sys_t;
...
static const char* notif_get_type_string(notif_type_t type) {
    switch (type) {
        case NOTIF_INFO:    return "Information";
        case NOTIF_WARNING: return "Warning";
        case NOTIF_ERROR:   return "Error";
...
static int notif_check_body(int index, int y, uint32_t mx, uint32_t my) {
    uint32_t x = notif.screen_w - NOTIF_WIDTH - NOTIF_PADDING;
    return (mx >= x && mx < x + NOTIF_WIDTH &&
            my >= (uint32_t)y && my < (uint32_t)(y + NOTIF_HEIGHT));
}
...
static void notif_open_modal(int index) {
    notif.modal_active = 1;
    notif.modal_source = notif.notifs[index];
    notif.modal_src_index = index;
    notif.modal_freeze_tick = timer_get_ticks();

    uint32_t title_len = strlen("Notification Details");
    uint32_t source_len = strlen("Source: ") + strlen(notif.modal_source.title);
    uint32_t message_len = strlen("Message: ") + strlen(notif.modal_source.message);
    uint32_t level_len = strlen("Level: ") + strlen(notif_get_type_string(notif.modal_source.type));

    uint32_t max_chars = title_len;
    if (source_len > max_chars) max_chars = source_len;
    if (message_len > max_chars) max_chars = message_len;
    if (level_len > max_chars) max_chars = level_len;

    notif.modal_w = (max_chars * 8) + 40; 
    if (notif.modal_w < 350) notif.modal_w = 350;
    if (notif.modal_w > notif.screen_w - 40) notif.modal_w = notif.screen_w - 40;

    notif.modal_h = 160;
    
    notif.modal_x = (notif.screen_w - notif.modal_w) / 2;
    notif.modal_y = (notif.screen_h - notif.modal_h) / 2;

    notif.modal_btn_w = 80;
    notif.modal_btn_h = 24;
    notif.modal_btn_x = notif.modal_x + (notif.modal_w - notif.modal_btn_w) / 2;
    notif.modal_btn_y = notif.modal_y + notif.modal_h - 35;

    vesa_mark_dirty(notif.modal_x, notif.modal_y, notif.modal_w, notif.modal_h);
}

static void notif_draw_modal(void) {
    if (!notif.modal_active) return;

    for (uint32_t j = 0; j < notif.modal_h; j++) {
        for (uint32_t i = 0; i < notif.modal_w; i++) {
            uint32_t px = notif.modal_x + i;
            uint32_t py = notif.modal_y + j;

            if (j < 25) {
                vesa_put_pixel(px, py, notif_get_color(notif.modal_source.type));
            } else {
                vesa_put_pixel(px, py, COLOR_MODAL_BG);
            }

            if (j == 0 || j == notif.modal_h - 1 || i == 0 || i == notif.modal_w - 1) {
                vesa_put_pixel(px, py, COLOR_BORDER);
            }
        }
    }

    vesa_draw_text(notif.modal_x + 10, notif.modal_y + 5, "Notification Details", 0xFFFFFF, notif_get_color(notif.modal_source.type));

    char buf[NOTIF_MAX_MSG + 16];
    
    sprintf(buf, "Source: %s", notif.modal_source.title);
    vesa_draw_text(notif.modal_x + 15, notif.modal_y + 40, buf, COLOR_TEXT, COLOR_MODAL_BG);

    sprintf(buf, "Message: %s", notif.modal_source.message);
    vesa_draw_text(notif.modal_x + 15, notif.modal_y + 60, buf, COLOR_TEXT, COLOR_MODAL_BG);

    sprintf(buf, "Level: %s", notif_get_type_string(notif.modal_source.type));
    vesa_draw_text(notif.modal_x + 15, notif.modal_y + 80, buf, COLOR_TEXT, COLOR_MODAL_BG);

    for (uint32_t j = 0; j < notif.modal_btn_h; j++) {
        for (uint32_t i = 0; i < notif.modal_btn_w; i++) {
            uint32_t px = notif.modal_btn_x + i;
            uint32_t py = notif.modal_btn_y + j;
            vesa_put_pixel(px, py, 0xD0D0D0);

            if (j == 0 || j == notif.modal_btn_h - 1 || i == 0 || i == notif.modal_btn_w - 1) {
                vesa_put_pixel(px, py, 0x707070);
            }
        }
    }
    vesa_draw_text(notif.modal_btn_x + 20, notif.modal_btn_y + 5, "Close", COLOR_TEXT, 0xD0D0D0);
...
static void notif_handle_clicks(void) {
    ...
    if (notif.modal_active) {
        if (mouse.buttons & 1) {
            ...
            if (notif.mouse_down) {
                if (mouse.x >= notif.modal_btn_x && mouse.x < notif.modal_btn_x + notif.modal_btn_w &&
                    mouse.y >= notif.modal_btn_y && mouse.y < notif.modal_btn_y + notif.modal_btn_h) {
                    
                    notif.modal_active = 0;
                    vesa_mark_dirty(notif.modal_x, notif.modal_y, notif.modal_w, notif.modal_h);
                    
                    if (notif.notifs[notif.modal_src_index].id == notif.modal_source.id) {
                        uint32_t duration = timer_get_ticks() - notif.modal_freeze_tick;
                        notif.notifs[notif.modal_src_index].created_tick += duration;
                    }
                }
                notif.mouse_down = 0;
            }
    ...
    if (mouse.buttons & 1) {
        if (!notif.mouse_down) {
            ...
            for (int i = 0; i < NOTIF_MAX_COUNT; i++) {
                ...
                if (y >= 0) {
                    ...
                    else if (notif_check_body(i, y, mouse.x, mouse.y)) {
                        notif.clicked_index = i;
                        notif.clicked_body = 1;
                        break;
                    }
                }
                visible++;
        ...
        if (notif.mouse_down && notif.clicked_index >= 0) {
            ...
            if (!notif.clicked_body && found_close == notif.clicked_index) {
                notif_remove(found_close);
            } 
            else if (notif.clicked_body && found_body == notif.clicked_index) {
                notif_open_modal(found_body);
            }
        }
        notif.mouse_down = 0;
        notif.clicked_index = -1;
...
void notif_printf(notif_type_t type, const char* title, const char* format, ...) {
    ...
    if (slot < 0) {
        for (int i = 0; i < NOTIF_MAX_COUNT; i++) {
            if (notif.notifs[i].id != 0 && (!notif.modal_active || notif.modal_src_index != i)) {
                notif_remove(i);
                break;
            }
        }
...
void notif_update(void) {
    ...
    for (int i = 0; i < NOTIF_MAX_COUNT; i++) {
        ...
        if (notif.modal_active && notif.modal_src_index == i) {
            continue; 
        }
