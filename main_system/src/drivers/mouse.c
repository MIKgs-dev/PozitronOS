...
static void ps2_write(uint8_t value) {
    ps2_wait(0);
    outb(0x64, 0xD4); // Команда "записать ведомому устройству PS/2 (мыши)"
    ps2_wait(0);
    outb(0x60, value);
}
...
void mouse_init(void) {
    ...
    // Включаем порт мыши на контроллере
    ps2_wait(0);
    outb(0x64, 0xA8);
    
    // Разрешаем прерывания в конфигурационном байте контроллера
    ps2_wait(0);
    outb(0x64, 0x20);
    ps2_wait(1);
    uint8_t config = inb(0x60);
    config |= 0x02; // Разрешить прерывания мыши (IRQ 12)
    config |= 0x01; // Разрешить прерывания клавиатуры (IRQ 1)
    ...
    // Сброс мыши на дефолтные настройки
    ps2_write(0xF6);
    ps2_read(); // Считываем ACK (0xFA)
    
    // Разрешаем мыши слать данные пакетов
    ps2_write(0xF4);
    ps2_read(); // Считываем ACK (0xFA)
...
void mouse_handler(registers_t* regs) {
    ...
    // Проверка 2: КРИТИЧЕСКАЯ ДЛЯ СОВМЕСТИМОСТИ. Если бита 5 (0x20) НЕТ,
    // значит данные пришли от клавиатуры! Игнорируем их в прерывании мыши.
    if (!(status & 0x20)) return;
    
    uint8_t data = inb(0x60);
    
    // Проверка 3: КРИТИЧЕСКАЯ ДЛЯ ТАЧПАДОВ. Валидация синхронизации байт-стрима.
    // Первый байт 3-байтового пакета PS/2 мыши ВСЕГДА содержит единицу в 3-м бите (маска 0x08).
    // Если мы на нулевом шаге, а бит сброшен — стрим рассинхронизировался. Сбрасываем счетчик.
    if (mouse.cycle == 0 && !(data & 0x08)) {
        return; 
    }
    ...
    if(mouse.cycle == 3) {
        ...
        // Повторная перестраховка структуры первого байта
        if(mouse.packet[0] & 0x08) {
            uint8_t old_buttons = mouse.public.buttons;
            mouse.public.buttons = mouse.packet[0] & 0x07;
            
            // Считаем дельту X с учетом знакового бита (бит 4 первого байта)
            int32_t dx = mouse.packet[1];
            if(mouse.packet[0] & 0x10) {
                dx -= 256;
            }
            
            // Считаем дельту Y с учетом знакового бита (бит 5 первого байта)
            int32_t dy = mouse.packet[2];
