...
static uint8_t keyboard_ready = 0;

#define KEYBOARD_QUEUE_SIZE 64

static keyboard_event_t keyboard_queue[KEYBOARD_QUEUE_SIZE];
static uint32_t keyboard_queue_head = 0;
static uint32_t keyboard_queue_tail = 0;
static uint32_t keyboard_queue_count = 0;

void keyboard_queue_push(uint8_t scancode, uint8_t ascii, uint8_t modifiers) {
    if (keyboard_queue_count >= KEYBOARD_QUEUE_SIZE) {
        keyboard_queue_head = (keyboard_queue_head + 1) % KEYBOARD_QUEUE_SIZE;
        keyboard_queue_count--;
    }
    
    keyboard_queue[keyboard_queue_tail].scancode = scancode;
    keyboard_queue[keyboard_queue_tail].ascii = ascii;
    keyboard_queue[keyboard_queue_tail].modifiers = modifiers;
    keyboard_queue_tail = (keyboard_queue_tail + 1) % KEYBOARD_QUEUE_SIZE;
    keyboard_queue_count++;
}

int keyboard_queue_pop(keyboard_event_t* event) {
    asm volatile("cli");
    ...
    *event = keyboard_queue[keyboard_queue_head];
    keyboard_queue_head = (keyboard_queue_head + 1) % KEYBOARD_QUEUE_SIZE;
    keyboard_queue_count--;
...
int keyboard_queue_available(void) {
...
static int is_printable(uint8_t scancode) {
    if (scancode >= 0x47 && scancode <= 0x53) return 0;
    if (scancode >= 0x3B && scancode <= 0x44) return 0;
...
static void keyboard_update_leds(void) {
    ...
    uint8_t led_byte = 0;
    if (kbd_state.scrolllock) led_byte |= 1;
    if (kbd_state.numlock)     led_byte |= 2;
    if (kbd_state.caps)        led_byte |= 4;
...
void keyboard_handler(registers_t* regs) {
    ...
    if (!released && is_printable(key)) {
        ...
        if (ascii >= 32 && ascii <= 126) {
            ...
            if (!KEY_RELEASED(key) && ascii > 0) {
                keyboard_queue_push(key, ascii, event.data2);
            }
        }
