...
uint32_t* wallpaper_pixels = NULL;
...
void vesa_enable_wc(void) {
    uint32_t base = (uint32_t)fb.address & 0xFFFFF000;
    uint32_t size_mb = (fb.width * fb.height * fb.bpp / 8) / (1024 * 1024) + 1;
    ...
    serial_puts("[VESA] Write-Combining enabled for LFB\n");
}
...
static inline void fast_copy_byte(uint8_t* dst, uint8_t* src, uint32_t bytes) {
    asm volatile (
        "rep movsb\n"
        : "+D" (dst), "+S" (src), "+c" (bytes)
        : 
...
static inline void fast_copy_word(uint16_t* dst, uint16_t* src, uint32_t words) {
    asm volatile (
        "rep movsw\n"
        : "+D" (dst), "+S" (src), "+c" (words)
        : 
...
void vesa_update_dirty(void) {
    ...
    for (uint32_t i = 0; i < dirty_count; i++) {
        ...
        if (r->x >= fb.width || r->y >= fb.height) continue;
        uint32_t width = r->w;
        if (r->x + width > fb.width) width = fb.width - r->x;
        uint32_t height = r->h;
        if (r->y + height > fb.height) height = fb.height - r->y;
        
        if (fb.bpp == 32) {
            for (uint32_t y = r->y; y < r->y + height; y++) {
                uint32_t* src = (uint32_t*)((uint8_t*)back_buffer + y * fb.pitch + r->x * 4);
                uint32_t* dst = (uint32_t*)((uint8_t*)fb.address + y * fb.pitch + r->x * 4);
                fast_copy(dst, src, width);
            }
        }
        else if (fb.bpp == 24) {
            uint32_t row_bytes = width * 3;
            uint32_t words = row_bytes / 4;
            uint32_t tail = row_bytes % 4;
            
            for (uint32_t y = r->y; y < r->y + height; y++) {
                uint32_t* src = (uint32_t*)((uint8_t*)back_buffer + y * fb.pitch + r->x * 3);
                uint32_t* dst = (uint32_t*)((uint8_t*)fb.address + y * fb.pitch + r->x * 3);
                
                if (words > 0) {
                    fast_copy(dst, src, words);
                }
                if (tail > 0) {
                    uint8_t* src8 = (uint8_t*)src + words * 4;
                    uint8_t* dst8 = (uint8_t*)dst + words * 4;
            ...
            for (uint32_t y = r->y; y < r->y + height; y++) {
                uint16_t* src = (uint16_t*)((uint8_t*)back_buffer + y * fb.pitch + r->x * 2);
                uint16_t* dst = (uint16_t*)((uint8_t*)fb.address + y * fb.pitch + r->x * 2);
...
void vesa_restore_background_dirty(void) {
    ...
    uint32_t bytes_per_pixel = fb.bpp / 8;
    uint32_t bytes_per_line = fb.width * bytes_per_pixel;
    ...
    for (uint32_t i = 0; i < dirty_count; i++) {
        ...
        uint32_t tail = copy_bytes % 4;
        
        for (uint32_t y = r->y; y < r->y + r->h && y < fb.height; y++) {
            uint8_t* src_ptr = cache + (y * bytes_per_line) + (r->x * bytes_per_pixel);
            uint8_t* dst_ptr = bb + (y * bytes_per_line) + (r->x * bytes_per_pixel);
            
            if (fb.bpp == 32) {
                fast_copy((uint32_t*)dst_ptr, (uint32_t*)src_ptr, words);
            } else {
                if (words > 0) {
                    fast_copy((uint32_t*)dst_ptr, (uint32_t*)src_ptr, words);
                }
                if (tail > 0) {
                    uint8_t* tail_src = src_ptr + (words * 4);
                    uint8_t* tail_dst = dst_ptr + (words * 4);
                    for (uint32_t b = 0; b < tail; b++) {
                        tail_dst[b] = tail_src[b];
                    }
                }
            }
        }
...
void vesa_swap_buffers(void) {
    ...
    if (fb.bpp == 32 && fb.pitch == bytes_per_line) {
        uint32_t total_words = fb.width * fb.height;
        fast_copy((uint32_t*)fb_ptr, (uint32_t*)bb_ptr, total_words);
    } 
    else {
        ...
        for (uint32_t y = 0; y < fb.height; y++) {
            uint32_t* dst32 = (uint32_t*)(fb_ptr + y * fb.pitch);
            uint32_t* src32 = (uint32_t*)(bb_ptr + y * bytes_per_line);
            if (words_per_line > 0) {
                fast_copy(dst32, src32, words_per_line);
            }

            if (rem > 0) {
                uint8_t* dst8 = (uint8_t*)(dst32 + words_per_line);
                uint8_t* src8 = (uint8_t*)(src32 + words_per_line);
...
void vesa_put_pixel(uint32_t x, uint32_t y, color_t color) {
    ...
    if (double_buffer_enabled && back_buffer) {
        buffer = (uint8_t*)back_buffer + y * fb.pitch + x * bytes_per_pixel;
    } else {
        buffer = (uint8_t*)fb.address + y * fb.pitch + x * bytes_per_pixel;
    }
    ...
    if (bytes_per_pixel == 4) {
        *(uint32_t*)buffer = color;
        ...
        uint8_t g = (color >> 8) & 0xFF;
        uint8_t b = color & 0xFF;
        *(uint16_t*)buffer = ((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3);
    }
}
...
int vesa_set_wallpaper(const char* path) {
    ...
    int res = vfs_open(path, FS_O_RDONLY, &file); 
    if (res < 0 || file == NULL) {
        serial_puts("[BMP] ERROR: Failed to open wallpaper file!\n");
        return -1;
    }
    ...
    vfs_lseek(file, 0, 0);
    vfs_read(file, &header, sizeof(BMPHeader), &bytes_read);

    vfs_lseek(file, 14, 0);
    vfs_read(file, &info, sizeof(BMPInfoHeader), &bytes_read);
    ...
    uint32_t img_h = (info.height > 0) ? info.height : -info.height;
    int is_inverted = (info.height > 0);

    uint32_t bytes_per_pixel = info.bpp / 8;
    uint32_t row_size = (img_w * bytes_per_pixel + 3) & ~3;
    uint32_t total_img_bytes = row_size * img_h;

    uint8_t* img_data = (uint8_t*)kmalloc(total_img_bytes); 
    if (!img_data) {
        vfs_close(file);
        return -4;
    }

    vfs_lseek(file, header.offset, 0); 
    vfs_read(file, img_data, total_img_bytes, &bytes_read);
    vfs_close(file);

    uint32_t* cache32 = (uint32_t*)background_cache;
    
    for (uint32_t screen_y = 0; screen_y < fb.height; screen_y++) {
        uint32_t img_y = ((2 * screen_y + 1) * img_h) / (2 * fb.height);
        ...
        uint8_t* img_row_ptr = img_data + (img_y * row_size);
        uint32_t screen_row_offset = screen_y * fb.width;

        for (uint32_t screen_x = 0; screen_x < fb.width; screen_x++) {
            uint32_t img_x = ((2 * screen_x + 1) * img_w) / (2 * fb.width);
            if (img_x >= img_w) img_x = img_w - 1;

            uint8_t* pixel_ptr = img_row_ptr + (img_x * bytes_per_pixel);
            uint32_t color = 0;
            ...
            cache32[screen_row_offset + screen_x] = color;
        }
    }

    kfree(img_data);
    serial_puts("[BMP] Universal scaling complete!\n");
    vesa_mark_dirty_all();
    return 0;
}
...
void vesa_clear_dirty_region(uint32_t x, uint32_t y, uint32_t w, uint32_t h) {
    ...
    for (uint32_t i = 0; i < dirty_count; i++) {
        ...
        if (x < r->x + r->w && x + w > r->x &&
            ...
            if (x <= r->x && y <= r->y && 
                x + w >= r->x + r->w && y + h >= r->y + r->h) {
                for (uint32_t j = i; j < dirty_count - 1; j++) {
                    dirty_rects[j] = dirty_rects[j + 1];
                }
                dirty_count--;
                i--;
            }
