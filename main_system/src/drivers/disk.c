...
static void disk_parse_mbr(disk_t* disk) {
    ...
    if (mbr[510] != 0x55 || mbr[511] != 0xAA) {
        serial_puts("[DISK] No valid MBR signature\n");
        disk->partition_count = 0;
        return;
    }
...
int disk_read_partition(disk_t* disk, int part_index, uint64_t lba, uint32_t count, void* buffer) {
    ...
    uint64_t part_end = (uint64_t)disk->partitions[part_index].start_lba + (uint64_t)disk->partitions[part_index].sector_count;
    
    if (real_lba + (uint64_t)count > part_end) {
        serial_puts("[DISK] Partition read out of bounds: real_lba=");
        serial_puts_num_ulong(real_lba);
        serial_puts(" count=");
        serial_puts_num(count);
        serial_puts(" part_end=");
        serial_puts_num_ulong(part_end);
        serial_puts("\n");
        return -1;
    }
...
int disk_write_partition(disk_t* disk, int part_index, uint64_t lba, uint32_t count, void* buffer) {
    ...
    if (real_lba + (uint64_t)count > part_end) {
        serial_puts("[DISK] Partition write out of bounds: real_lba=");
        serial_puts_num_ulong(real_lba);
        serial_puts(" count=");
        serial_puts_num(count);
        serial_puts(" part_end=");
