...
static cursor_type_t determine_cursor_type(void) {
    ...
    if (win->resizable && !win->maximized && !win->minimized) {
        ...
        // Углы
        // Верхний левый угол
        if (mx >= win->x && mx <= win->x + corner_size &&
            my >= win->y && my <= win->y + corner_size) {
            return CURSOR_RESIZE_NWSE;
        }
        // Верхний правый угол
        if (mx >= win->x + win->width - corner_size && mx < win->x + win->width &&
            my >= win->y && my <= win->y + corner_size) {
            return CURSOR_RESIZE_NESW;
        }
        // Нижний левый угол
        if (mx >= win->x && mx <= win->x + corner_size &&
            my >= win->y + win->height - corner_size && my < win->y + win->height) {
            return CURSOR_RESIZE_NESW;
        }
        // Нижний правый угол
        if (mx >= win->x + win->width - corner_size && mx < win->x + win->width &&
        ...
        // Края
        // Левый край
        if (mx >= win->x && mx <= win->x + edge_size &&
            my >= win->y + title_h && my < win->y + win->height) {
            return CURSOR_RESIZE_EW;
        }
        // Правый край
        if (mx >= win->x + win->width - edge_size && mx < win->x + win->width &&
            my >= win->y + title_h && my < win->y + win->height) {
            return CURSOR_RESIZE_EW;
        }
        // Нижний край
        if (my >= win->y + win->height - edge_size && my < win->y + win->height &&
    ...
    if (win->has_titlebar && win->movable && !win->maximized) {
        ...
        if (my >= title_y && my < title_y + title_h && mx >= win->x && mx < win->x + win->width) {
    ...
    while (widget) {
        if (widget->visible && widget->enabled &&
            ...
            switch (widget->type) {
                case WIDGET_INPUT:   return CURSOR_TEXT;
                case WIDGET_BUTTON:
                case WIDGET_CHECKBOX: return CURSOR_HAND;
                case WIDGET_SLIDER:  return CURSOR_RESIZE_EW;
                case WIDGET_SCROLLBAR: {
                    ScrollbarData* sb = (ScrollbarData*)widget->data;
                    if (sb) return sb->vertical ? CURSOR_RESIZE_NS : CURSOR_RESIZE_EW;
                    break;
                }
                case WIDGET_LIST:
                case WIDGET_DROPDOWN: return CURSOR_HAND;
                default: break;
            }
            break;
        }
