...
static hw_device_t device_pool[MAX_DEVICES];
static uint32_t device_pool_count = 0;
...
// ============ ДОБАВЛЕНИЕ УСТРОЙСТВА В ПУЛ ============
static hw_device_t* alloc_device(void) {
    if (device_pool_count >= MAX_DEVICES) {
        serial_puts("[SCAN] ERROR: Device pool full!\n");
        return NULL;
    }
    
    hw_device_t* hw = &device_pool[device_pool_count++];
...
static void pci_add_device(uint8_t bus, uint8_t dev, uint8_t func) {
    ...
    *d = '\0';
    ...
    put_hex(device, 4);
    serial_puts("] at ");
    if(bus < 10) serial_puts("0");
    serial_puts_num(bus);
    serial_puts(":");
    if(dev < 10) serial_puts("0");
    serial_puts_num(dev);
    serial_puts(".");
...
void scanner_deinit(void) {
    // Просто сбрасываем указатели — память освободится при перезагрузке
    device_list = NULL;
    device_counter = 0;
    device_pool_count = 0;
...
void scanner_dump_pci(void) {
    ...
    while(dev) {
        if(dev->bus == BUS_PCI) {
            serial_puts("  ");
            if(dev->pci.bus < 10) serial_puts("0");
            serial_puts_num(dev->pci.bus);
            serial_puts(":");
            if(dev->pci.device < 10) serial_puts("0");
            serial_puts_num(dev->pci.device);
            serial_puts(".");
            serial_puts_num(dev->pci.function);
            serial_puts(" ");
