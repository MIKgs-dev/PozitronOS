...
static int ext2_disk_read(disk_t* disk, uint64_t lba, uint32_t count, void* buf) {
    int rc = disk_read(disk, lba, count, buf);
    if (rc != 0) {
        serial_puts("[EXT2] Disk read ERROR: lba=");
        serial_puts_num_ulong(lba);
        serial_puts(" count=");
...
static int ext2_disk_write(disk_t* disk, uint64_t lba, uint32_t count, void* buf) {
    int rc = disk_write(disk, lba, count, buf);
    if (rc != 0) {
        serial_puts("[EXT2] Disk write ERROR: lba=");
        serial_puts_num_ulong(lba);
        serial_puts(" count=");
        serial_puts_num(count);
        serial_puts(" rc=");
...
static int ext2_read_block(struct ext2_private* priv, uint32_t block, void* buf) {
    uint32_t block_size_in_sectors = priv->block_size / 512;
    uint32_t sector = priv->disk->partition_offset + block * block_size_in_sectors;
    return ext2_disk_read(priv->disk, sector, block_size_in_sectors, buf);
}

static int ext2_write_block(struct ext2_private* priv, uint32_t block, void* buf) {
    uint32_t sector = priv->disk->partition_offset + block * (priv->block_size / 512);
    uint32_t sectors = priv->block_size / 512;
    return ext2_disk_write(priv->disk, sector, sectors, buf);
}
...
static uint32_t ext2_alloc_inode(struct ext2_private* priv, uint32_t group) {
    ...
    for (i = 0; i < priv->inodes_per_group; i++) {
        if (i + 1 < priv->sb.s_first_ino) continue;
...
static int ext2_read_block_from_inode(struct ext2_private* priv, struct ext2_inode* inode, 
    ...
    *block = 0;
    ...
    block_num -= per_block * per_block;

    if (block_num < per_block * per_block * per_block) {
        if (inode->i_block[14] == 0) return -1;
        if (ext2_read_block(priv, inode->i_block[14], triple_indirect) != 0) return -1;
        
        i = block_num / (per_block * per_block);
        if (triple_indirect[i] == 0) return -1;
        
        if (ext2_read_block(priv, triple_indirect[i], double_indirect) != 0) return -1;
        
        uint32_t j = (block_num % (per_block * per_block)) / per_block;
        if (double_indirect[j] == 0) return -1;
        
        if (ext2_read_block(priv, double_indirect[j], indirect) != 0) return -1;
...
// Новая функция с буфером
static int ext2_read_block_from_inode_buf(struct ext2_private* priv, struct ext2_inode* inode, 
                                           uint32_t block_num, uint32_t* block, void* buf) {
    uint32_t* indirect = (uint32_t*)buf;
    uint32_t* double_indirect = (uint32_t*)buf;
    uint32_t* triple_indirect = (uint32_t*)buf;
    uint32_t i;
    uint32_t per_block = priv->block_size / 4;
    ...
    *block = 0;
    ...
    if (block_num < per_block) {
        if (inode->i_block[12] == 0) return -1;
        if (ext2_read_block(priv, inode->i_block[12], buf) != 0) return -1;
        *block = indirect[block_num];
        return 0;
    }
    ...
    if (block_num < per_block * per_block) {
        if (inode->i_block[13] == 0) return -1;
        if (ext2_read_block(priv, inode->i_block[13], buf) != 0) return -1;
        
        i = block_num / per_block;
        if (double_indirect[i] == 0) return -1;
        
        if (ext2_read_block(priv, double_indirect[i], buf) != 0) return -1;
...
static int ext2_write_block_to_inode(struct ext2_private* priv, struct ext2_inode* inode,
    ...
    if (block_num < per_block * per_block) {
        ...
        uint32_t i = block_num / per_block;
...
static int ext2_write_data(struct ext2_private* priv, struct ext2_inode* inode,
    ...
    uint32_t to_write;
    int new_block;
    uint32_t current_block_in_buf = 0xFFFFFFFF;
    ...
    while (written < size) {
        ...
        current_block_in_buf = 0xFFFFFFFF;
...
static int ext2_sync(struct vfs_superblock* sb) {
    struct ext2_private* priv = (struct ext2_private*)sb->private_data;
    if (priv && priv->disk && priv->disk->flush) {
        return priv->disk->flush(priv->disk);
    }
    return 0;
}

static int ext2_lookup(vfs_inode_t* dir, const char* name, vfs_inode_t** result) {
    struct ext2_private* priv = (struct ext2_private*)dir->private_data;
    if (!priv) return -1;
    
    struct ext2_inode dir_inode;
    if (ext2_read_inode(priv, dir->i_ino, &dir_inode) != 0) return -1;
    
    uint8_t* local_block_buf = kmalloc(priv->block_size);
    if (!local_block_buf) return -1;
    
    uint32_t offset = 0;
    while (offset < dir_inode.i_size) {
        uint32_t block_num = offset / priv->block_size;
        uint32_t block_offset = offset % priv->block_size;
        uint32_t block;
        
        if (ext2_read_block_from_inode(priv, &dir_inode, block_num, &block) != 0 || block == 0) {
            offset += priv->block_size;
            continue;
        }
        
        if (ext2_read_block(priv, block, local_block_buf) != 0) {
            offset += priv->block_size;
            continue;
        }
        
        uint32_t entry_offset = block_offset;
        while (entry_offset < priv->block_size) {
            struct ext2_dir_entry* entry = (struct ext2_dir_entry*)(local_block_buf + entry_offset);
            ...
            if (entry->inode != 0 && entry->name_len == strlen(name) &&
                ...
                if (ext2_read_inode(priv, entry->inode, &file_inode) != 0) {
                    kfree(local_block_buf);
                    return -1;
                }
                ...
                *result = kmalloc(sizeof(vfs_inode_t));
                ...
                (*result)->i_mode = file_inode.i_mode;
                (*result)->i_size = file_inode.i_size;
                (*result)->i_blocks = file_inode.i_blocks;
                (*result)->private_data = priv;
                (*result)->sb = dir->sb;
                
                (*result)->fops = &ext2_fops;
                (*result)->iops = &ext2_iops;
...
static int ext2_write(vfs_file_t* file, const void* buf, uint32_t size, uint32_t* bytes_written) {
    ...
    file->f_inode->i_size = inode.i_size;
    file->f_inode->i_blocks = inode.i_blocks;
...
static int ext2_readdir(vfs_file_t* file, void* dirent, uint32_t* bytes_read) {
    ...
    if (ext2_read_block_from_inode(priv, &inode, block_num, &block) != 0 || block == 0) {
        file->f_pos = offset + priv->block_size;
        *bytes_read = 0;
        return 0;
    }
    ...
    uint32_t entry_offset = block_offset;
    while (entry_offset < priv->block_size) {
        struct ext2_dir_entry* entry = (struct ext2_dir_entry*)(priv->block_buf + entry_offset);
        
        if (entry->rec_len == 0) {
            file->f_pos = offset + (priv->block_size - block_offset);
            *bytes_read = 0;
            return 0;
        }
        
        if (entry->rec_len > priv->block_size - entry_offset) {
            entry_offset += 8;
            continue;
        }
        ...
        if (entry->inode != 0) {
            ...
            file->f_pos = offset + (entry_offset - block_offset) + entry->rec_len;
...
static int ext2_close(vfs_file_t* file) {
    struct ext2_private* priv = (struct ext2_private*)file->f_inode->private_data;
    if (!priv) return -1;
    
    struct ext2_inode inode;
    if (ext2_read_inode(priv, file->f_inode->i_ino, &inode) == 0) {
        inode.i_size = file->f_inode->i_size;
        inode.i_blocks = file->f_inode->i_blocks;
        ext2_write_inode(priv, file->f_inode->i_ino, &inode);
    }
...
static int ext2_create(vfs_inode_t* dir, const char* name, uint32_t mode, vfs_inode_t** result) {
    ...
    rec_len = sizeof(struct ext2_dir_entry) + strlen(name);
    ...
    entry = (struct ext2_dir_entry*)(priv->block_buf + entry_offset);
    if (entry->rec_len >= rec_len) {
        uint32_t total_write_len = entry->rec_len; // Сохраняем исходную длину свободного куска
        ...
        if (new_rec_len >= 8) {
            ...
            entry->rec_len = total_write_len;
        }
        
        uint32_t block_start_offset = (offset / priv->block_size) * priv->block_size;
        
        if (offset + priv->block_size > parent_inode.i_size) {
            parent_inode.i_size = block_start_offset + priv->block_size;
        }

        if (ext2_write_data(priv, &parent_inode, block_start_offset,
                            priv->block_buf, priv->block_size, &bytes) != 0) {
            ext2_free_inode(priv, ino);
            return -1;
        }
...
static int ext2_mkdir(vfs_inode_t* dir, const char* name, uint32_t mode) {
    ...
    offset = 0;
    while (offset < parent_inode.i_size) {
        uint32_t check_block_num = offset / priv->block_size;
        uint32_t check_block_offset = offset % priv->block_size;
        uint32_t check_block;
        
        if (ext2_read_block_from_inode(priv, &parent_inode, check_block_num, &check_block) != 0 || check_block == 0) {
            offset += priv->block_size;
            continue;
        }
        
        if (ext2_read_block(priv, check_block, priv->block_buf) != 0) {
            offset += priv->block_size;
            continue;
        }
        
        uint32_t check_entry_offset = check_block_offset;
        while (check_entry_offset < priv->block_size) {
            entry = (struct ext2_dir_entry*)(priv->block_buf + check_entry_offset);
            if (entry->rec_len == 0) break;
            ...
            check_entry_offset += entry->rec_len;
        }
        offset += priv->block_size;
    }
    
    // Выделяем inode для новой директории
    ino = ext2_alloc_inode(priv, 0);
    if (ino == 0) return -1;
    ...
    // Создаём содержимое новой директории: "." и ".."
    memset(priv->block_buf, 0, priv->block_size);
    ...
    // Выделяем локальный буфер для чтения родительской директории
    local_buf = kmalloc_aligned(priv->block_size, priv->block_size);
    if (!local_buf) {
        ext2_free_block(priv, block);
        ext2_free_inode(priv, ino);
        return -1;
    ...
    // Ищем место в родительской директории
    found_space = 0;
    uint32_t num_blocks = (parent_inode.i_size + priv->block_size - 1) / priv->block_size;
    
    for (uint32_t block_idx = 0; block_idx < num_blocks && !found_space; block_idx++) {
        uint32_t read_block;
        // Используем версию с буфером!
        if (ext2_read_block_from_inode_buf(priv, &parent_inode, block_idx, &read_block, local_buf) != 0 || read_block == 0) {
            continue;
        }
        
        entry_offset = 0;
        while (entry_offset < priv->block_size) {
            entry = (struct ext2_dir_entry*)(local_buf + entry_offset);
            
            if (entry->rec_len == 0) break;
            
            // Случай 1: Свободная запись
            if (entry->inode == 0 && entry->rec_len >= rec_len) {
                found_space = 1;
                target_block_num = block_idx;
                target_block = read_block;
                target_offset = entry_offset;
                break;
            }
            
            // Случай 2: Занятая запись, проверяем хвостовое свободное пространство
            if (entry->inode != 0) {
                uint32_t real_size = sizeof(struct ext2_dir_entry) + entry->name_len;
                uint32_t aligned_size = (real_size + 3) & ~3;
                uint32_t free_space = entry->rec_len - aligned_size;
                
                if (free_space >= rec_len) {
                    // Уменьшаем rec_len текущей записи прямо в local_buf
                    entry->rec_len = aligned_size;
                    
                    found_space = 1;
                    target_block_num = block_idx;
                    target_block = read_block;
                    target_offset = entry_offset + aligned_size;
                    break;
                }
            }
    ...
    if (!found_space) {
        // Нет места в существующих блоках - создаём новый блок в конце файла
        uint32_t new_block_num = num_blocks;
        uint32_t new_block = ext2_alloc_block(priv, 0);
        if (new_block == 0) {
            kfree_aligned(local_buf);
            ext2_free_block(priv, block);
            ext2_free_inode(priv, ino);
            return -1;
        }
        
        if (ext2_write_block_to_inode(priv, &parent_inode, new_block_num, new_block) != 0) {
            kfree_aligned(local_buf);
            ext2_free_block(priv, new_block);
            ext2_free_block(priv, block);
            ext2_free_inode(priv, ino);
        ...
        memset(local_buf, 0, priv->block_size);
        entry = (struct ext2_dir_entry*)local_buf;
        entry->inode = 0;
        entry->rec_len = priv->block_size;
        ...
        parent_inode.i_size += priv->block_size;
        if (ext2_write_inode(priv, dir->i_ino, &parent_inode) != 0) {
            kfree_aligned(local_buf);
            ext2_free_block(priv, block);
            ext2_free_inode(priv, ino);
            return -1;
    ...
    // Копируем из local_buf в block_buf для записи
    memcpy(priv->block_buf, local_buf, priv->block_size);
    kfree_aligned(local_buf);
    
    // Вставляем новую запись
    entry = (struct ext2_dir_entry*)(priv->block_buf + target_offset);
    if (entry->rec_len >= rec_len) {
        uint32_t new_rec_len = entry->rec_len - rec_len;
        ...
        if (ext2_write_block(priv, target_block, priv->block_buf) != 0) {
            ext2_free_block(priv, block);
            ext2_free_inode(priv, ino);
            return -1;
        }
        ...
        if (dir->sb && dir->sb->sops && dir->sb->sops->sync) {
            dir->sb->sops->sync(dir->sb);
        }
...
static vfs_superblock_operations_t ext2_sops = {
    .read_inode = NULL,
    .write_inode = NULL,
    .sync = ext2_sync,
};
...
static void ext2_cleanup_private(struct ext2_private* priv) {
    ...
    if (priv->groups) {
        uint32_t groups_size = priv->groups_count * sizeof(struct ext2_group_desc);
        kfree_dma_region(priv->groups, groups_size);
        priv->groups = NULL;
    }
    if (priv->block_buf) {
...
static int ext2_mount(disk_t* disk, vfs_superblock_t** sb) {
    ...
    uint32_t groups_phys;
    int part_index;
    uint32_t partition_offset;
    
    part_index = disk_find_partition_by_type(disk, EXT2_PARTITION_TYPE);
    if (part_index < 0) return -1;

    partition_offset = disk->partitions[part_index].start_lba;
    disk->partition_offset = partition_offset;
    ...
    sb_buf = kmalloc_aligned(1024, 512);
    if (!sb_buf) {
        kfree_aligned(priv);
        return -1;
    }

    if (disk_read_partition(disk, part_index, 2, 2, sb_buf) != 0) {
    ...
    groups_size = priv->groups_count * sizeof(struct ext2_group_desc);
    
    priv->groups = (struct ext2_group_desc*)kmalloc_dma_region(groups_size, &groups_phys);
    if (!priv->groups) {
        kfree_aligned(sb_buf);
        kfree_aligned(priv);
        return -1;
    ...
    memset(priv->groups, 0, groups_size);
    ...
    for (i = 0; i < group_desc_blocks; i++) {
        ...
        if (disk_read_partition(disk, part_index, sector, priv->block_size / 512, (uint8_t*)priv->groups + offset) != 0) {
            kfree_dma_region(priv->groups, groups_size);
            kfree_aligned(sb_buf);
            kfree_aligned(priv);
            return -1;
    ...
    if (!priv->block_buf || !priv->inode_buf || !priv->bitmap_buf) {
        kfree_dma_region(priv->groups, groups_size);
        if (priv->block_buf) kfree_aligned(priv->block_buf);
        if (priv->inode_buf) kfree_aligned(priv->inode_buf);
        if (priv->bitmap_buf) kfree_aligned(priv->bitmap_buf);
        kfree_aligned(sb_buf);
        kfree_aligned(priv);
        return -1;
    }
...
static int ext2_format_group(struct ext2_private* priv, uint32_t group) {
    ...
    memset(buf, 0, priv->block_size);
    if (group == 0) {
        for (i = 0; i < priv->sb.s_first_ino - 1; i++) {
            ext2_set_bit(buf, i, 1);
        }
    }
...
int ext2_format(disk_t* disk) {
    ...
    for (i = 0; i < groups; i++) {
        ...
        block += (inodes_per_group * sb.s_inode_size + block_size - 1) / block_size;
    ...
    uint32_t root_inode_num = ext2_alloc_inode(&priv, 0);
    if (root_inode_num != EXT2_ROOT_INO) {
        kfree(buf);
        return -1;
    }
...
int ext2_check(disk_t* disk) {
    struct ext2_superblock sb;
    if (ext2_disk_read(disk, 2, 2, &sb) != 0) return 0;
