        ...
        while (widget) {
            if (widget->type == WIDGET_LABEL && widget->text) {
                ...
                if (is_time_widget) {
                    ...
                    if (widget->userdata) {
                        gui_strncpy((char*)widget->userdata, new_time, 9);
                    }
...
static void create_date_menu(void) {
    ...
    wg_create_label(date_menu_window, "Current time:", 0.1f, 0.25f);
    Widget* time_label = wg_create_label(date_menu_window, time_str, 0.1f, 0.40f);
...
void start_menu_create(void) {
    ...
    start_menu_window = wm_create_window("PozitronOS | Start Menu",
                                        menu_x, menu_y,
                                        250, 250,
                                        WINDOW_HAS_TITLE);
    ...
    wg_create_button(start_menu_window, "Power Options", 
                    0.1f, 0.8f, 0.8f, 0.1f,
...
void taskbar_handle_event(event_t* event) {
    if (!taskbar_initialized || taskbar_disabled) return;
        ...
        if (taskbar_find_button_at(mx, my, &clicked_window)) {
            if (clicked_window == NULL) {
                ...
                if (!IS_VALID_WINDOW_PTR(clicked_window)) {
                    for (uint32_t i = 0; i < MAX_TASKBAR_BUTTONS; i++) {
                        if (taskbar_buttons[i].window == clicked_window) {
                            taskbar_buttons[i].valid = 0;
                            taskbar_buttons[i].window = NULL;
                            taskbar_buttons[i].title[0] = '\0';
                        }
...
void taskbar_enable(void) {
    if (taskbar_initialized) {
        taskbar_disabled = 0;
        update_taskbar_geometry();
        vesa_mark_dirty(0, gui_state.screen_height - TASKBAR_HEIGHT, 
                        gui_state.screen_width, TASKBAR_HEIGHT);
        serial_puts("[TASKBAR] Enabled\n");
    }
}
...
void taskbar_disable(void) {
    if (taskbar_initialized) {
        taskbar_disabled = 1;
        vesa_draw_rect(0, gui_state.screen_height - TASKBAR_HEIGHT, 
                       gui_state.screen_width, TASKBAR_HEIGHT, 0x000000);
        vesa_mark_dirty(0, gui_state.screen_height - TASKBAR_HEIGHT, 
                        gui_state.screen_width, TASKBAR_HEIGHT);
        serial_puts("[TASKBAR] Disabled\n");
    }
}