...
extern uint8_t taskbar_disabled;
...
Widget* gui_get_widget_by_id(uint32_t id) {
    Window* win = gui_state.first_window;
    while (win) {
...
static void handle_input_special_key(Widget* input, uint8_t key, uint8_t modifiers) {
    ...
    uint8_t shift = modifiers & 0x01;
    uint8_t ctrl = modifiers & 0x02;
    uint32_t len = gui_strlen(data->buffer);
    uint32_t old_pos = data->cursor_pos;
    ...
    switch (key) {
        ...
        case 0x4B: // Left arrow
            if (!shift && !ctrl) {
                ...
                // Ctrl+Left — слово влево (пока пропускаем)
                wg_input_cursor_left(input);
                data->selection_start = data->cursor_pos;
                data->selection_end = data->cursor_pos;
            }
...
static void handle_input_mouse_click(Widget* input, int32_t mx, int32_t my, uint8_t shift) {
    ...
    if (!shift) {
        // Без Shift — сбрасываем выделение
        data->selection_start = char_pos;
        data->selection_end = char_pos;
    } else {
        // С Shift — расширяем выделение от текущего start
        data->selection_end = char_pos;
        // Если нужно, можно менять и start, если кликнули левее start
        if (char_pos < data->selection_start) {
            data->selection_start = char_pos;
            data->selection_end = data->cursor_pos;
        }
    }
...
static void handle_input_mouse_drag(Widget* input, int32_t mx, int32_t my) {
    ...
    // Если перетащили левее start — swap
    if (data->selection_end < data->selection_start) {
        uint32_t tmp = data->selection_start;
        data->selection_start = data->selection_end;
        data->selection_end = tmp;
    }
...
void gui_handle_event(event_t* event) {
    ...
    if (event->type == EVENT_KEY_PRESS) {
        ...
        Window* focused_win = wm_get_focused_window();
        if (focused_win && focused_win->input_enabled) {
            input_event_t input_ev;
            input_ev.type = POZ_EVENT_KEY_DOWN;
            input_ev.window_id = focused_win->id;
            input_ev.timestamp = event->timestamp;
            input_ev.key.scancode = event->key.scancode;
            input_ev.key.ascii = event->key.ascii;
            input_ev.key.modifiers = event->key.modifiers;
            wm_push_input_event(focused_win, input_ev);
        }
        
        if (gui_state.focus_widget && gui_state.focus_widget->handle_event) {
            gui_state.focus_widget->handle_event(gui_state.focus_widget, event);
        }
    ...
    if (event->type == EVENT_MOUSE_CLICK && button == 0) {
        Window* mouse_win = wm_find_window_at(mx, my);
        if (mouse_win && mouse_win->input_enabled) {
            input_event_t input_ev;
            input_ev.type = POZ_EVENT_MOUSE_BUTTON;
            input_ev.window_id = mouse_win->id;
            input_ev.timestamp = event->timestamp;
            input_ev.mouse.x = mx - mouse_win->x;
            input_ev.mouse.y = my - mouse_win->y;
            input_ev.mouse.delta_x = 0;
            input_ev.mouse.delta_y = 0;
            input_ev.mouse.buttons = button + 1;  // 1=левая, 2=правая, 4=средняя
            input_ev.mouse.wheel = 0;
            wm_push_input_event(mouse_win, input_ev);
        }
        ...
        if (mouse_win && mouse_win->input_enabled) {
            input_event_t input_ev;
            input_ev.type = POZ_EVENT_MOUSE_MOVE;
            input_ev.window_id = mouse_win->id;
            input_ev.timestamp = event->timestamp;
            input_ev.mouse.x = mx - mouse_win->x;
            input_ev.mouse.y = my - mouse_win->y;
            input_ev.mouse.delta_x = mx - mouse_win->x - last_mouse_x;  // нужно хранить
            input_ev.mouse.delta_y = my - mouse_win->y - last_mouse_y;
            input_ev.mouse.buttons = mouse_get_buttons();
            input_ev.mouse.wheel = 0;
            wm_push_input_event(mouse_win, input_ev);
        }
        ...
        if (mouse_win && mouse_win->input_enabled) {
            input_event_t input_ev;
            input_ev.type = POZ_EVENT_MOUSE_WHEEL;
            input_ev.window_id = mouse_win->id;
            input_ev.timestamp = event->timestamp;
            input_ev.mouse.x = mx - mouse_win->x;
            input_ev.mouse.y = my - mouse_win->y;
            ...
            input_ev.mouse.delta_y = 0;
            input_ev.mouse.buttons = mouse_get_buttons();
            input_ev.mouse.wheel = (event->data2 & 0x80000000) ? -1 : 1;
            wm_push_input_event(mouse_win, input_ev);
        }
    ...
    if (has_visible_windows) {
        ...
        for (uint32_t i = 0; i < count; i++) {
            ...
            if (!window->needs_redraw && !window->dragging && !window->resizing) {
                if (!window->canvas_enabled || !window->canvas_front) {
                    continue;
                }
            }
            ...
            if (window->canvas_enabled && window->canvas_front) {
                uint32_t dest_y_base = window->y + window->title_height;
                
                uint32_t w = window->canvas_width;
                uint32_t h = window->canvas_height;
                
                if (w > window->width) w = window->width;
                if (h > window->height - window->title_height) 
                    h = window->height - window->title_height;
                
                if (w > 0 && h > 0) {
                    for (uint32_t y = 0; y < h; y++) {
                        uint32_t* src_row = window->canvas_front + y * window->canvas_width;
                        for (uint32_t x = 0; x < w; x++) {
                            vesa_put_pixel(window->x + x, dest_y_base + y, src_row[x]);
                        }
                    }
                }
                
                window->canvas_swap_pending = 0;
            }
                    ...
                    switch (widget->type) {
                        case WIDGET_BUTTON: {
                            ...
                            if (widget->text) {
                                ...
                                calculate_text_position(widget, widget->text, &text_x, &text_y);
                        ...
                        case WIDGET_LABEL:
                            if (widget->text) {
                                uint32_t text_x, text_y;
                                calculate_text_position(widget, widget->text, &text_x, &text_y);
                                vesa_draw_text(text_x, text_y, widget->text, 0x000000, WINDOW_BG_COLOR);
                            }
                            break;
                        ...
                        case WIDGET_INPUT: {
                            widget->focused = (widget == gui_state.focus_widget) ? 1 : 0;
