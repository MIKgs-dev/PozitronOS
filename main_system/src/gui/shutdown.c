...
#define SHUTDOWN_STATE_IDLE          0
#define SHUTDOWN_STATE_DIALOG        1
#define SHUTDOWN_STATE_CANCELING     2
#define SHUTDOWN_STATE_CONFIRMING    3

static Window* original_windows[64];
uint32_t original_window_count = 0;
uint8_t shutdown_state = SHUTDOWN_STATE_IDLE;
static Window* shutdown_dialog = NULL;
uint8_t darken_level = 0;
uint8_t shutdown_immediate = 0;
    ...
    switch (shutdown_state) {
        ...
        case SHUTDOWN_STATE_CANCELING:
            ...
            if (darken_level == 0) {
                ...
                if (gui_state.first_window) {
                    gui_state.focused_window = gui_state.first_window; 
                    gui_state.first_window->focused = 1;
                }
                gui_force_redraw();
                vesa_mark_dirty_all();
        ...
        case SHUTDOWN_STATE_CONFIRMING:
            if (darken_level < 200) {
                ...
                if (darken_level >= 200 && !shutdown_immediate) {
                    ...
                    serial_puts("[SHUTDOWN] Complete black - calling action\n");
...
void show_verification_dialog(void);

static void power_options_callback(Widget* button, void* userdata) {
    (void)button;
    uint32_t action = (uint32_t)userdata;
    
    if (action == 0) {
        shutdown_cancel_callback(button, shutdown_dialog);
        return;
    }
...
static void verification_back_callback(Widget* button, void* userdata) {
    (void)button;
    (void)userdata;
    
    shutdown_dialog_callback(NULL, NULL);
}
...
void show_verification_dialog(void) {
    ...
    shutdown_dialog = wm_create_window("Verification",
                                      dialog_x, dialog_y,
                                      dialog_width, dialog_height,
                                      WINDOW_HAS_TITLE);
    if (!shutdown_dialog) return;
    ...
    if (selected_action == 1) {
        wg_create_label(shutdown_dialog, "Are you sure you want to", 0.24f, 0.25f);
        wg_create_label(shutdown_dialog, "shutdown the computer?", 0.23f, 0.45f);
    } else {
        wg_create_label(shutdown_dialog, "Are you sure you want to", 0.24f, 0.25f);
        wg_create_label(shutdown_dialog, "reboot the computer?", 0.25f, 0.45f);
    }
    
    wg_create_button(shutdown_dialog, "Yes", 0.25f, 0.70f, 0.2f, 0.2f, shutdown_confirm_callback, NULL);
    wg_create_button(shutdown_dialog, "No", 0.55f, 0.70f, 0.2f, 0.2f, verification_back_callback, NULL);
...
void shutdown_dialog_callback(Widget* button, void* userdata) {
    ...
    if (shutdown_state != SHUTDOWN_STATE_IDLE && shutdown_state != SHUTDOWN_STATE_DIALOG) {
        return;
    }
    ...
    shutdown_dialog = wm_create_window("Power Options",
                                      dialog_x, dialog_y,
                                      dialog_width, dialog_height,
                                      WINDOW_HAS_TITLE);
    ...
    wg_create_label(shutdown_dialog, "Select power action", 0.28f, 0.30f);
    
    wg_create_button(shutdown_dialog, "Shutdown", 0.08f, 0.65f, 0.26f, 0.2f, power_options_callback, (void*)1);
    wg_create_button(shutdown_dialog, "Reboot",   0.37f, 0.65f, 0.26f, 0.2f, power_options_callback, (void*)2);
    wg_create_button(shutdown_dialog, "Return",   0.66f, 0.65f, 0.26f, 0.2f, power_options_callback, (void*)0);
