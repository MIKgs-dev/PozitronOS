...
void wm_do_resize(Window* window, int32_t mouse_x, int32_t mouse_y) {
    ...
    if (window->x == new_x && window->y == new_y && 
        window->width == (uint32_t)new_width && window->height == (uint32_t)new_height) {
        return;
    }
    ...
    // Обновляем канвас
    if (window->canvas_enabled) {
        uint32_t new_canvas_width = window->width;
        uint32_t new_canvas_height = window->height - window->title_height;
        
        uint32_t needed = new_canvas_width * new_canvas_height;
        if (needed <= window->canvas_capacity) {
            window->canvas_width = new_canvas_width;
            window->canvas_height = new_canvas_height;
            
            for (uint32_t i = 0; i < needed; i++) {
                if (window->canvas_back) window->canvas_back[i] = 0x00000000;
                if (window->canvas_front) window->canvas_front[i] = 0x00000000;
            }
            window->canvas_swap_pending = 0;
        } else {
            uint32_t* new_back = (uint32_t*)kmalloc(needed * sizeof(uint32_t));
            uint32_t* new_front = (uint32_t*)kmalloc(needed * sizeof(uint32_t));
            
            if (new_back && new_front) {
                for (uint32_t i = 0; i < needed; i++) {
                    new_back[i] = 0x00000000;
                    new_front[i] = 0x00000000;
                }
                
                if (window->canvas_back) kfree(window->canvas_back);
                if (window->canvas_front) kfree(window->canvas_front);
                
                window->canvas_back = new_back;
                window->canvas_front = new_front;
                window->canvas_width = new_canvas_width;
                window->canvas_height = new_canvas_height;
                window->canvas_capacity = needed;
                window->canvas_swap_pending = 0;
            }
        }
    }
...
Window* wm_create_window(const char* title, uint32_t x, uint32_t y, 
    ...
    window->canvas_back = NULL;
    window->canvas_front = NULL;
    window->canvas_width = 0;
    window->canvas_height = 0;
    window->canvas_capacity = 0;
    window->canvas_enabled = 0;
    window->canvas_swap_pending = 0;
    ...
    window->win_event_head = 0;
    window->win_event_tail = 0;
    window->win_event_count = 0;

    for (int i = 0; i < 128; i++) {
        window->input_queue[i].type = 0;
    }
    window->input_head = 0;
    window->input_tail = 0;
    window->input_count = 0;
    window->input_enabled = 1;
...
void wm_destroy_window(Window* window) {
    ...
    // Очистка канваса
    if (window->canvas_back) {
        kfree(window->canvas_back);
        window->canvas_back = NULL;
    }
    if (window->canvas_front) {
        kfree(window->canvas_front);
        window->canvas_front = NULL;
    }
    window->canvas_width = 0;
    window->canvas_height = 0;
...
void wm_close_window(Window* window) {
    ...
    uint32_t pid_to_kill = window->owner_pid;
    uint32_t win_id = window->id;

    safe_remove_window_from_list(window);
    gui_unregister_window(win_id);
    ...
    if (gui_state.focused_window == window) {
        gui_state.focused_window = NULL;
        if (gui_state.last_window && IS_VALID_WINDOW_PTR(gui_state.last_window)) {
            gui_state.focused_window = gui_state.last_window;
            gui_state.focused_window->focused = 1;
            gui_state.focused_window->needs_redraw = 1;
        }
    }
...
void wm_maximize_window(Window* window) {
    ...
    // Обновляем канвас при максимизации
    if (window->canvas_enabled) {
        uint32_t new_canvas_width = window->width;
        uint32_t new_canvas_height = window->height - window->title_height;
...
void wm_restore_window(Window* window) {
    ...
    if (window->minimized) {
        ...
        // Обновляем канвас при восстановлении
        if (window->canvas_enabled) {
            uint32_t new_canvas_width = window->width;
            uint32_t new_canvas_height = window->height - window->title_height;
...
void wm_destroy_windows_by_pid(uint32_t pid) {
    if (pid == 0) return;
    ...
    while (window) {
        ...
        if (IS_VALID_WINDOW_PTR(window) && window->id != 0 && window->owner_pid == pid) {
            window->owner_pid = 0; 
...
void wm_post_to_window(Window* win, event_t ev) {
    if (!win) return;
    
    asm volatile("cli");
    
    if (win->win_event_count >= WIN_EVENT_QUEUE_SIZE) {
        win->win_event_head = (win->win_event_head + 1) % WIN_EVENT_QUEUE_SIZE;
        win->win_event_count--;
    }
    
    win->win_events[win->win_event_tail] = ev;
    win->win_event_tail = (win->win_event_tail + 1) % WIN_EVENT_QUEUE_SIZE;
    win->win_event_count++;
    
    asm volatile("sti");
}

void wm_push_input_event(Window* win, input_event_t ev) {
    if (!win || !win->input_enabled) return;
    
    if (win->input_count >= 128) {
        win->input_head = (win->input_head + 1) % 128;
        win->input_count--;
    }
    
    win->input_queue[win->input_tail] = ev;
    win->input_tail = (win->input_tail + 1) % 128;
    win->input_count++;
}

void wm_cleanup_all_windows(void) {
    serial_puts("[WM] Starting full cleanup...\n");
    
    uint32_t freed_windows = 0;
    uint32_t freed_widgets = 0;
    
    Window* window = gui_state.first_window;
    while (window) {
        Window* next_window = window->next;
        
        if (IS_VALID_WINDOW_PTR(window) && window->id != 0) {
            Widget* widget = window->first_widget;
            while (widget) {
                freed_widgets++;
                widget = widget->next;
    ...
    serial_puts("[WM] Cleanup complete: freed ");
    serial_puts_num(freed_windows);
    serial_puts(" windows and ");
    serial_puts_num(freed_widgets);
    serial_puts(" widgets\n");
