...
static Widget* create_widget_base(Window* parent, WidgetType type) {
    ...
    Widget* widget = (Widget*)kmalloc((sizeof(Widget) + 63) & ~63);
    ...
    widget->text_align = TEXT_ALIGN_LEFT | TEXT_ALIGN_TOP;
    if (type == WIDGET_BUTTON) widget->text_align = TEXT_ALIGN_CENTER | TEXT_ALIGN_MIDDLE;
...
Widget* wg_create_button_aligned(Window* parent, const char* text,
                                float rel_x, float rel_y, 
                                float rel_width, float rel_height,
                                uint8_t align,
                                void (*callback)(Widget*, void*), void* userdata) {
    Widget* widget = wg_create_button(parent, text, rel_x, rel_y, rel_width, rel_height, callback, userdata);
    if (widget) widget->text_align = align;
    return widget;
}
...
void calculate_text_position(Widget* widget, const char* text, uint32_t* out_x, uint32_t* out_y) {
    if (!widget || !text || !out_x || !out_y) return;

    uint32_t text_len_px = gui_strlen(text) * 8;
    uint32_t font_height = 16;
    uint32_t padding = 2;

    if (widget->type == WIDGET_BUTTON || widget->type == WIDGET_CHECKBOX || 
        widget->type == WIDGET_INPUT || widget->type == WIDGET_LIST || 
        widget->type == WIDGET_DROPDOWN || widget->type == WIDGET_TAB ||
        widget->type == WIDGET_PROGRESSBAR || widget->type == WIDGET_SLIDER ||
        widget->type == WIDGET_SCROLLBAR || widget->type == WIDGET_MENUBAR) {
        
        if (widget->text_align & TEXT_ALIGN_CENTER) {
            *out_x = widget->x + (widget->width - text_len_px) / 2;
        } else if (widget->text_align & TEXT_ALIGN_RIGHT) {
            *out_x = widget->x + widget->width - text_len_px - padding;
        } else {
            *out_x = widget->x + padding;
        }

        if (widget->text_align & TEXT_ALIGN_MIDDLE) {
            *out_y = widget->y + (widget->height - font_height) / 2;
        } else if (widget->text_align & TEXT_ALIGN_BOTTOM) {
            *out_y = widget->y + widget->height - font_height - padding;
        } else {
            *out_y = widget->y + padding;
        }
    } else {
        if (widget->text_align & TEXT_ALIGN_CENTER) {
            uint32_t win_width = widget->parent_window ? widget->parent_window->width : gui_state.screen_width;
            *out_x = widget->parent_window ? widget->parent_window->x + (win_width - text_len_px) / 2 : (gui_state.screen_width - text_len_px) / 2;
        } else if (widget->text_align & TEXT_ALIGN_RIGHT) {
            uint32_t win_width = widget->parent_window ? widget->parent_window->width : gui_state.screen_width;
            *out_x = widget->parent_window ? widget->parent_window->x + win_width - text_len_px - padding : gui_state.screen_width - text_len_px - padding;
        } else {
            *out_x = widget->x + padding;
        }

        if (widget->text_align & TEXT_ALIGN_MIDDLE) {
            uint32_t win_height = widget->parent_window ? widget->parent_window->height : gui_state.screen_height;
            *out_y = widget->parent_window ? widget->parent_window->y + (win_height - font_height) / 2 : (gui_state.screen_height - font_height) / 2;
        } else if (widget->text_align & TEXT_ALIGN_BOTTOM) {
            uint32_t win_height = widget->parent_window ? widget->parent_window->height : gui_state.screen_height;
            *out_y = widget->parent_window ? widget->parent_window->y + win_height - font_height - padding : gui_state.screen_height - font_height - padding;
        } else {
            *out_y = widget->y + padding;
        }
    }
...
Widget* wg_create_label_aligned(Window* parent, const char* text,
                                float rel_x, float rel_y, uint8_t align) {
    Widget* widget = wg_create_label(parent, text, rel_x, rel_y);
    if (widget) {
        widget->text_align = align;
    }
    return widget;
...
void wg_set_text(Widget* widget, const char* text) {
    ...
    uint32_t new_len = gui_strlen(text);
    
    if (widget->text) {
        uint32_t old_len = gui_strlen(widget->text);
        if (new_len <= old_len) {
            gui_strcpy(widget->text, text);
            widget->needs_redraw = 1;
            if (widget->parent_window && IS_VALID_WINDOW_PTR(widget->parent_window)) {
    ...
    widget->text = (char*)kmalloc(new_len + 1);
    if (widget->text) {
        gui_strcpy(widget->text, text);
        widget->needs_redraw = 1;
        if (widget->parent_window && IS_VALID_WINDOW_PTR(widget->parent_window)) {
