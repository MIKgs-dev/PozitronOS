...
static const uint32_t slab_sizes[SLAB_CACHE_COUNT] = {64, 128, 256, 384, 512, 768, 1024, 2048};
...
static void* legacy_kmalloc(uint32_t size);
static void legacy_kfree(void* ptr);
static void* legacy_krealloc(void* ptr, uint32_t size);
...
static void slab_init(void) {
    for (int i = 0; i < SLAB_CACHE_COUNT; i++) {
        ...
        slab_caches[i].pages_total = 0;

        uint32_t bitmap_size = ((SLAB_PAGE_SIZE - sizeof(slab_page_t)) / (slab_sizes[i] + 1) + 31) / 32 * 4;
        uint32_t data_area = SLAB_PAGE_SIZE - sizeof(slab_page_t) - bitmap_size;
        slab_caches[i].objs_per_page = data_area / slab_sizes[i];
        
        if (slab_caches[i].objs_per_page > 128) slab_caches[i].objs_per_page = 128;
    }
    serial_puts("[MEM] SLAB allocator initialized (");
    serial_puts_num(SLAB_CACHE_COUNT);
    serial_puts(" caches, max obj 2KB)\n");
}

static slab_page_t* slab_page_alloc(slab_cache_t* cache) {
    slab_page_t* page = (slab_page_t*)legacy_kmalloc(SLAB_PAGE_SIZE);
    if (!page) return NULL;
    ...
    uint32_t bitmap_size = (cache->objs_per_page + 7) / 8;
    page->bitmap = (uint32_t*)((uint8_t*)page + sizeof(slab_page_t));
    page->data = (uint8_t*)page->bitmap + bitmap_size;
...
static void* slab_alloc(uint32_t size) {
    if (size > SLAB_MAX_OBJ) return NULL;
        ...
        if (slab_sizes[i] >= size) {
            ...
            while (page) {
                if (page->free_count > 0) {
                    for (uint32_t j = 0; j < cache->objs_per_page; j++) {
                        if (!(page->bitmap[j / 32] & (1 << (j % 32)))) {
                            page->bitmap[j / 32] |= (1 << (j % 32));
                            page->free_count--;

                            if (page->free_count == 0) {
                                if (prev) prev->next = page->next;
                                else cache->partial = page->next;

                                page->next = cache->full;
                                cache->full = page;
                            }
                            return page->data + j * page->obj_size;
                        }
                    }
                }
                prev = page;
...
static void slab_free(void* ptr) {
    ...
    for (int i = 0; i < SLAB_CACHE_COUNT; i++) {
        ...
        slab_page_t** list_ptr = &cache->full;
        while (*list_ptr) {
            slab_page_t* page = *list_ptr;
            uint32_t offset = (uint32_t)((uint8_t*)ptr - page->data);
            if (offset < page->obj_size * cache->objs_per_page) {
                uint32_t idx = offset / page->obj_size;
                page->bitmap[idx / 32] &= ~(1 << (idx % 32));
                page->free_count++;
        ...
        while (*list_ptr) {
            ...
            if (offset < page->obj_size * cache->objs_per_page) {
                ...
                if (page->free_count == cache->objs_per_page) {
                    if (prev) prev->next = page->next;
                    else cache->partial = page->next;
                    legacy_kfree(page);
                    cache->pages_total--;
...
static uint32_t get_slab_obj_size(void* ptr) {
    if (!ptr) return 0;

    for (int i = 0; i < SLAB_CACHE_COUNT; i++) {
        slab_cache_t* cache = &slab_caches[i];

        slab_page_t* page = cache->full;
        while (page) {
            uint32_t offset = (uint32_t)((uint8_t*)ptr - page->data);
            if (offset < page->obj_size * cache->objs_per_page) {
...
void* krealloc(void* ptr, uint32_t size) {
    ...
    uint32_t slab_size = get_slab_obj_size(ptr);
    if (slab_size > 0) {
        old_size = slab_size;
        is_slab = 1;
    } else {
...
void memory_init(void) {
    ...
    serial_puts("[MEM] Using dual-level allocator (SLAB + Legacy)\n");
