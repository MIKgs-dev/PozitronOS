...
static uint8_t reboot_countdown_active = 0;
static uint32_t reboot_countdown = 15;
static uint32_t reboot_countdown_tick = 0;
static Widget* reboot_countdown_label = NULL;
static Widget* reboot_countdown_progress = NULL;
...
static void reboot_now_callback(Widget* button, void* userdata) {
    (void)button;
    (void)userdata;
    serial_puts("[INSTALLER] User requested immediate reboot\n");
    acpi_reboot();
}

static void shutdown_now_callback(Widget* button, void* userdata) {
    (void)button;
    (void)userdata;
    serial_puts("[INSTALLER] User requested immediate shutdown\n");
    acpi_poweroff();
}

static void update_reboot_countdown(void) {
    if (!reboot_countdown_active) return;
    if (current_stage != STAGE_COMPLETE) return;
    
    uint32_t current_tick = timer_get_ticks();
    uint32_t elapsed_ticks = current_tick - reboot_countdown_tick;
    
    if (elapsed_ticks >= 100) {
        uint32_t seconds_passed = elapsed_ticks / 100;
        reboot_countdown_tick += seconds_passed * 100;
        
        if (seconds_passed >= reboot_countdown) {
            reboot_countdown = 0;
            seconds_passed = reboot_countdown;
            reboot_countdown_active = 0;
            
            if (reboot_countdown_label) {
                wg_set_text(reboot_countdown_label, 
                    "Computer will restart in 0 seconds");
            }
            if (reboot_countdown_progress) {
                wg_set_progressbar_value(reboot_countdown_progress, 0);
            }
            if (main_window) {
            ...
            serial_puts("[INSTALLER] Countdown finished, rebooting...\n");
        ...
        reboot_countdown -= seconds_passed;
        
        if (reboot_countdown_label) {
            char countdown_text[64];
            if (reboot_countdown == 1) {
                sprintf(countdown_text, 
                    "Computer will restart in 1 second");
            } else {
                sprintf(countdown_text, 
                    "Computer will restart in %d seconds", reboot_countdown);
            }
            wg_set_text(reboot_countdown_label, countdown_text);
        }
        
        if (reboot_countdown_progress) {
            uint32_t seconds_elapsed = 15 - reboot_countdown;
            uint32_t progress_value = (seconds_elapsed * 100) / 15;
            wg_set_progressbar_value(reboot_countdown_progress, progress_value);
        }
        ...
        serial_puts("[INSTALLER] Countdown: ");
        serial_puts_num(reboot_countdown);
        serial_puts(" seconds\n");
    }
}

static void completion_message(void) {
    serial_puts("[INSTALLER] Installation complete. Preparing reboot screen...\n");
    ...
    wg_create_label_aligned(main_window, 
        "System installed successfully!", 
        0.5f, 0.25f, TEXT_ALIGN_CENTER);
    
    reboot_countdown_progress = wg_create_progressbar(
        main_window, 0.1f, 0.40f, 0.8f, 0.06f, 0);
    
    reboot_countdown_label = wg_create_label_aligned(main_window,
        "Computer will restart in 15 seconds",
        0.5f, 0.50f, TEXT_ALIGN_CENTER);
    
    wg_create_button(main_window, "Restart now",
        0.55f, 0.85f, 0.20f, 0.07f,
        reboot_now_callback, NULL);
    
    wg_create_button(main_window, "Shutdown now",
        0.77f, 0.85f, 0.20f, 0.07f,
        shutdown_now_callback, NULL);
    ...
    reboot_countdown = 15;
    reboot_countdown_tick = timer_get_ticks();
    reboot_countdown_active = 1;
...
static void perform_installation(void) {
    ...
    uint32_t write_sectors_per_block = 4096;
...
static void start_installation(Widget* button, void* userdata) {
    ...
    wg_create_label_aligned(main_window, "Installing PozitronOS...", 0.5f, 0.20f, TEXT_ALIGN_CENTER);
...
static void welcome_install_callback(Widget* button, void* userdata) {
    ...
    wg_create_label_aligned(main_window, "Select target disk", 0.5f, 0.10f, TEXT_ALIGN_CENTER);
    ...
    if (disk_index == 0) {
        wg_create_label_aligned(main_window, "No hard disks found!", 0.5f, 0.35f, TEXT_ALIGN_CENTER);
        wg_create_label_aligned(main_window, "Please check your hardware and try again.", 0.5f, 0.43f, TEXT_ALIGN_CENTER);
    }
...
static void create_welcome_stage(Window* win) {
    wg_create_label_aligned(win, "PozitronOS Setup", 0.5f, 0.25f, TEXT_ALIGN_CENTER);
    wg_create_label_aligned(win, "This wizard will install PozitronOS on your computer.", 0.5f, 0.45f, TEXT_ALIGN_CENTER);
    wg_create_label_aligned(win, "Make sure you have backed up your data before continuing.", 0.5f, 0.52f, TEXT_ALIGN_CENTER);
...
void kernel_main(uint32_t magic, multiboot_info_t* mb_info) {
    ...
    if (system_img_size == 0) {
        ...
        if (error_win) {
            ...
            error_win->movable = 0;
            wg_create_label_aligned(error_win, "No system image found!", 0.5f, 0.35f, TEXT_ALIGN_CENTER);
            wg_create_label_aligned(error_win, "Please check your installation media.", 0.5f, 0.55f, TEXT_ALIGN_CENTER);
        }
    } else {
