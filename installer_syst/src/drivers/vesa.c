...
void vesa_update_dirty(void) {
    ...
    for (uint32_t i = 0; i < dirty_count; i++) {
        ...
        if (fb.bpp == 32) {
            ...
            for (uint32_t y = r->y; y < r->y + height; y++) {
                uint8_t* src = (uint8_t*)back_buffer + y * fb.pitch + r->x * 3;
                uint8_t* dst = (uint8_t*)fb.address + y * fb.pitch + r->x * 3;
...
void vesa_restore_background_dirty(void) {
    ...
    for (uint32_t i = 0; i < dirty_count; i++) {
        ...
        for (uint32_t y = r->y; y < r->y + r->h && y < fb.height; y++) {
            uint8_t* src_ptr = cache + (y * fb.width * bytes_per_pixel) + (r->x * bytes_per_pixel);
            uint8_t* dst_ptr = bb + (y * fb.pitch) + (r->x * bytes_per_pixel);
...
void vesa_swap_buffers(void) {
    ...
    if (fb.bpp == 32 && fb.pitch == bytes_per_line) {
        ...
        for (uint32_t y = 0; y < fb.height; y++) {
            ...
            uint32_t* src32 = (uint32_t*)(bb_ptr + y * fb.pitch);
